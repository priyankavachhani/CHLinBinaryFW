...
//    ortp_log(severity, "(%*s:%-4d) %s", FILE_SIZE, filename + MAX((int)strlen(filename) - FILE_SIZE, 0), line, utf8str);
    ...
    //NSLog(@"log details : %s  :  %s",filename ,utf8str);
	va_end(args);
}
...
#ifdef USE_CRASHLYTHICSS
				CLS//NSLog(@"[%@] %@%@", lvl, tab, (NSString *)myWords[i]);
#else
				////NSLog(@"[%@] %@%@", lvl, tab, (NSString *)myWords[i]);
                //NSLog(@"%@%@",  tab, (NSString *)myWords[i]);
#endif
...
#ifdef USE_CRASHLYTHICSS
		CLS//NSLog(@"[%@] %@", lvl, [formatedString stringByReplacingOccurrencesOfString:@"\r\n" withString:@"\n"]);
#else
