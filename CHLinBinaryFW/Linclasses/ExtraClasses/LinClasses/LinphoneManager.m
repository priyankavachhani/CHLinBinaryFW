...
- (id)init {
    if ((self = [super init])) {
        ...
        _linphoneManagerAddressBookMap = [[NSDictionary alloc] init];
        pushCallIDs = [[NSMutableArray alloc] init];
//        _isTesting = [LinphoneManager instanceRelease];
        _isTesting = false;
...
- (void) setContactsUpdated:(BOOL) updated{                 // Trushang_delete
    _contactsUpdated = updated;
}
- (BOOL) getContactsUpdated{                                // Trushang_delete
...
- (void)migrateFromUserPrefs {
    ...
    @{ @"backgroundmode_preference" : @NO,
       ...
       @"start_at_boot_preference" : @NO };
    BOOL shouldSync = FALSE;
    
    LOGI(@"Trushang_migrateFromUserPrefs :   %lu user prefs", (unsigned long)[defaults_keys count]);
        ...
        while (proxies) {
            ...
            if (addr &&
                strstr(addr, [LinphoneManager.instance lpConfigStringForKey:@"domain_name"
                    ...
                    linphone_proxy_config_enable_avpf(proxy, FALSE);
                }
...
- (void)migratePushNotificationPerAccount {
    ...
    if (s && s.boolValue) {
        ...
        [self lpConfigSetBool:YES forKey:@"pushnotification_preference"];
...
//                    NSString *hello = [NSString stringWithUTF8String:linphone_call_get_call_log(call)];
//                    //NSLog(@"Chetan Dictionary %@",linphone_call_log_get_from_address(linphone_call_get_call_log(call)));
                    ...
                    //NSLog(@"Test _ 10");
                    //NSLog(@"xphfromtransferNumber 1 : %s",(linphone_call_get_to_header(call,"X-PH-To")));
                    //NSLog(@"xphfromtransferNumber 2 :  %s",(linphone_call_get_to_header(call,"X-PH-From")));
                    //NSLog(@"xphfromtransferNumber 3 :  %s",(linphone_call_get_to_header(call,"X-PH-Fromnumber")));
                    //NSLog(@"xphfromtransferNumber 4 :  %s",(linphone_call_get_to_header(call,"X-PH-toTransferNumber")));
                    //NSLog(@"xphfromtransferNumber 5 :  %s",(linphone_call_get_to_header(call,"X-PH-Networkstrength")));
                    //NSLog(@"xphfromtransferNumber 6 :  %s",(linphone_call_get_to_header(call,"X-PH-Transferid")));
                    //NSLog(@"xphfromtransferNumber 7 :  %s",(linphone_call_get_to_header(call,"X-PH-fromTransferNumber")));
                    //NSLog(@"xphfromtransferNumber 8 : %s",(linphone_call_get_to_header(call,"X-PH-Callhold")));
                    //NSLog(@"xphfromtransferNumber 9 :  %s",(linphone_call_get_to_header(call,"X-PH-Calltransfer")));
...
//                    NSData *decodedData = [[NSData alloc] initWithBase64EncodedString:[NSString stringWithUTF8String:linphone_call_get_to_header(call,"X-PH-From")] options:0];
//                    NSString *xphfrom = [[NSString alloc] initWithData:decodedData encoding:NSUTF8StringEncoding];
//                    //NSLog(@"Decode String Value: %@", xphfrom);
...
//                    xphfrom = [xphfrom stringByRemovingPercentEncoding];
                    ...
                    NSString *xphfrom = @"";
                     ...
                     //NSLog(@"Test _ 13");
                    xphfrom = [xphfrom stringByRemovingPercentEncoding];
                    if([self validateString:xphfrom withPattern:@"^[0-9]+$"])
                    {
                        NSString *str = @"+";
                         //NSLog(@"Test _ 14");
                        xphfrom = [NSString stringWithFormat:@"%@%@",str,xphfrom];
                    }
                    ...
                    XPHFromnumber = [XPHFromnumber stringByRemovingPercentEncoding];
                    if([self validateString:XPHFromnumber withPattern:@"^[0-9]+$"])
                    {
                        NSString *str = @"+";
                        XPHFromnumber = [NSString stringWithFormat:@"%@%@",str,XPHFromnumber];
                    }
...
//                     {if (error) LOGD(@"Error while adding notification request : %@", error.description);}];
                ...
                if (proxyCfg) {
                    ...
                    pushNotifEnabled = 1;//(refkey && strcmp(refkey, "push_notification") == 0);
                }
            ...
            if (uuid) {
                ...
                if (callKit_callId && !_conf) {
                    ...
                    CXHandle *handle = [[CXHandle alloc] initWithType:CXHandleTypePhoneNumber value:address];
...
- (void)onGlobalStateChanged:(LinphoneGlobalState)state withMessage:(const char *)message {
    LOGI(@"Trushang_onGlobalStateChanged: %d (message: %s)", state, message);
...
//            ? [NSString stringWithFormat:@"%@ : %@", fromMsg, @""]
//            : @"";
//        } else {
//            content.body = strcmp(subject, LINPHONE_DUMMY_SUBJECT) != 0
...
void networkReachabilityCallBack(SCNetworkReachabilityRef target, SCNetworkReachabilityFlags flags, void *nilCtx) {
    ...
    if (theLinphoneCore != nil) {
        ...
        if ((flags == 0) || (flags & networkDownFlags)) {
            ...
            BOOL isWifiOnly = false; // [lm lpConfigBoolForKey:@"wifi_only_preference" withDefault:FALSE];
            if (!ctx || ctx->testWWan)
...
- (void)finishCoreConfiguration {
    ...
    NSString *device1 = [NSString stringWithFormat:@"CallHippo-ios-LinphoneSDK"];
    NSString *version = [NSString stringWithFormat:@"%s",LINPHONE_SDK_VERSION_AGENT];
    //NSLog(@"Trushang Device Name get : %@",device1);

    
    linphone_core_set_user_agent(theLinphoneCore, device1.UTF8String, version.UTF8String);
...
//    linphone_core_set_root_ca(theLinphoneCore, [LinphoneManager bundleFile:@"pushcert.pem"].UTF8String);
    NSString *cerpath = [[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"voippushcer.cer"];
    linphone_core_set_user_certificates_path(theLinphoneCore, cerpath.UTF8String);
...
- (BOOL)enterBackgroundMode {
    ...
    if (proxyCfg) {
        const char *refkey = proxyCfg ? linphone_proxy_config_get_ref_key(proxyCfg) : NULL;
        BOOL pushNotifEnabled = 1;//(refkey && strcmp(refkey, "push_notification") == 0);
        if ([LinphoneManager.instance lpConfigBoolForKey:@"backgroundmode_preference"] || pushNotifEnabled) {
            if (floor(NSFoundationVersionNumber) <= NSFoundationVersionNumber_iOS_9_x_Max) {
...
- (BOOL)doCallWithSas:(const LinphoneAddress *)iaddr isSas:(BOOL)isSas {
    ...
    NSString* firstName = [NSString stringWithFormat: @"%@", userid];
    NSString* firstName2 = [NSString stringWithFormat: @"%@", selno];
    NSString* firstName3 = [NSString stringWithFormat: @"iOS"];
    
    
    const char* seconduserid = [firstName UTF8String];
    const char* secondselno = [firstName2 UTF8String];
    const char* Devicetype = [firstName3 UTF8String];
    const char* reminder = [strReminderId UTF8String];
    
    
    //NSLog(@"----------------%@-----------------%s",strReminderId,reminder);
    //NSLog(@"\n \n BCBC_secondselno: %s \n \n ",secondselno);

    linphone_address_set_header(addr, "X-PH-Userid", seconduserid);
    //linphone_address_set_header(addr, "X-PH-Fromnumber", secondselno);
    linphone_address_set_header(addr, "X-PH-Devicetype", Devicetype);
    //linphone_address_set_header(addr, "X-PH-reminderId", reminder);
    
     //NSLog(@"Trushang : Callsetup :");
     //NSLog(@"X-PH-Userid : %s",seconduserid);
     //NSLog(@"X-PH-Fromnumber : %s",secondselno);
     //NSLog(@"X-PH-Devicetype : %s",Devicetype);
     //NSLog(@"X-PH-reminderId : %s",reminder);
...
- (void)setPushNotificationToken:(NSData *)apushNotificationToken {
    ...
    //NSLog(@"_pushNotificationToken  : %@",apushNotificationToken);
    @try {
        const MSList *proxies = linphone_core_get_proxy_config_list(LC);
        while (proxies) {
            [self configurePushTokenForProxyConfig:proxies->data];
...
- (void)configurePushTokenForProxyConfig:(LinphoneProxyConfig *)proxyCfg {
    ...
    BOOL pushNotifEnabled =   1;//(refkey && strcmp(refkey, "push_notification") == 0);
    //NSLog(@"Trushang_patel : tokenData : %@",tokenData);
    //NSLog(@"Trushang_patel : pushNotifEnabled : %@",pushNotifEnabled ? @"Yes" : @"No");
    if (tokenData != nil && pushNotifEnabled) {
        //NSLog(@"Trushang_patel %@ ",tokenData);
...
//                .lastPathComponent;
        ...
        //NSLog(@"Cer mode : %@",APPMODE_SUFFIX);
                NSString *params = [NSString
                    stringWithFormat:@"pn-type=apple;pn-tok=%@",
                                      tokenString];
//        NSString *params = [NSString
...
//                            [[NSBundle mainBundle] bundleIdentifier], APPMODE_SUFFIX, tokenString, ring, timeout];
        ...
        LOGI(@"Proxy config %s configured for push notifications with contact: %@",
             linphone_proxy_config_get_identity(proxyCfg), @"");
