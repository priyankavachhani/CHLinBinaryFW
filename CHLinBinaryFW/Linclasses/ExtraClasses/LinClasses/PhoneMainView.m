...
//- (PhoneMainView *)setViewControllerForDescription:(UIView *)description {
//    return currentViewController;
//
		...
		case LinphoneCallStreamsRunning: {
			...
			if (floor(NSFoundationVersionNumber) > NSFoundationVersionNumber_iOS_9_x_Max && call) {
				...
				if (uuid) {
					...
					update.remoteHandle = [[CXHandle alloc] initWithType:CXHandleTypePhoneNumber value:address];
...
- (void)startUp {
	@try {
		LinphoneManager *lm = LinphoneManager.instance;
                LOGI(@"Hello Chetan %s", linphone_global_state_to_string(
